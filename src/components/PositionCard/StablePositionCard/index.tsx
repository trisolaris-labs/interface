...
export default function FullStablePositionCard({ poolName, border }: StablePositionCardProps) {
  ...
  const { address: poolAddress, poolTokens: nonTlpTokens } = STABLESWAP_POOLS[poolName]
  const poolTokens = tokens.map(({ token }) => token)

  const currencies = nonTlpTokens.map(token => unwrappedToken(token))
  const [showMore, setShowMore] = useState(false)

  const backgroundColor1 = useColorWithDefault('#2172E5', nonTlpTokens[0])
  const backgroundColor2 = useColorWithDefault('#2172E5', nonTlpTokens[nonTlpTokens.length - 1])
    ...
    <StyledPositionCard border={border} bgColor={backgroundColor1} id={`stableswap-position-card-${name}`}>
      ...
      <AutoColumn gap="8px">
        <StyledFixedHeightRow onClick={handleCardClick} id={`stableswap-compact-clickable-position-card-${name}`}>
          <RowFixed>
            {currencies.length > 2 ? (
              <MultipleLogo currencies={currencies} size={20} />
            ) : (
              <DoubleCurrencyLogo currency0={currencies[0]} currency1={currencies[1]} size={20} />
            )}
