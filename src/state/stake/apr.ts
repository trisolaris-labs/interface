...
import { STAKING, StakingTri, TRI, rewardsPerSecond, totalAllocPoints, tokenAmount, ExternalInfo } from './stake-constants'
import {
...
export function useFarms(): StakingTri[] {
  ...
  return useMemo(() => {
    ...
    return lpAddresses.reduce<StakingTri[]>((memo, lpAddress, index) => {
      ...
      if (
        ...
        const totalSupplyStaked = JSBI.BigInt(stakingInfoData[index].totalStaked)
        ...
        const usdStaked = Math.round(stakingInfoData[index].totalStakedInUSD)
        const totalStakedAmountInUSD = new TokenAmount(usdc, JSBI.BigInt(usdStaked)) // TO REPLACE
        // apr calculation
        ...
        const apr = stakingInfoData[index].apr // TO REPLACE
        console.log('APR: ' + apr)
        ...
        memo.push({
          ...
          totalStakedAmountInETH: activeFarms[index].totalStakedAmountInETH, // remove from meo
  ...
  }, [chainId, activeFarms, lpAddresses, userInfo, pendingTri, pairs, pairTotalSupplies, stakingInfoData, usdc])
}
